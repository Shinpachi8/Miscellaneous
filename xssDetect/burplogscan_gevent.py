...
def exploit(inqueue, outqueue, delay):
    while True:
        if inqueue.empty():
            break
        hj = inqueue.get()
                    ...
                    if p == 'xss':
                        for regex in XSS_Rule[p]:
                            ...
                            if regex in content and status_code == 200 and headers.get('Content-Type', '').split(';')[0]  not in  ["application/json", "text/plain", "application/javascript", "text/json", "text/javascript", "application/x-javascript"]:
                                ...
                                outqueue.put(('XSS', payload, hj.response.request.url))
                                found = True
                                break
                    if p == 'lfi':
                        if "root:x:0" in content and status_code == 200:
                            outqueue.put(('LFI', payload, hj.response.request.url))
                            found = True
                            break
                    if p == 'redirect':
                        if 'Valar Morghulis' in content and status_code == 200:
                            outqueue.put(('Unsafe Redirect', payload, hj.response.request.url))
                            found = True
                            break

                    if p == 'ssti':
                        if '3351376549499229720' in content and status_code == 200:
                            outqueue.put(('SSTI', payload, hj.response.request.url))
                            found = True
                            break
